...
public:
	...
	void* LookUp(const unsigned int value);
	void findParent(LLTB_t* pNode, unsigned int key, int parent = 0);
	LLTB_t* getRoot() const;
	bool isRed(LLTB_t* pNode, unsigned int key, bool& colorChoice);


	bool Insert(VoidRef_t ref);
	LLTB_t* InsertRec(LLTB_t *pNode, VoidRef_t ref);

	void Delete(const unsigned int value);
	LLTB_t* DeleteRec(LLTB_t *pNode, const unsigned int value);
	LLTB_t* DeleteMin(LLTB_t *pNode);

	void SanityCheck(void);
	void SanityCheckRec(LLTB_t *pNode, bool isParentRed, unsigned int blackDepth, unsigned int &minBlack, unsigned int &maxBlack);

	unsigned int  KeyCount(void);
	unsigned int  KeyCountRec(LLTB_t *pNode);

	void LeafDepth(void);
	unsigned int  LeafDepthRec(LLTB_t *pNode, unsigned int depth, unsigned int ary[], unsigned int depthLimit, unsigned int blackDepth, unsigned int &minBlack, unsigned int &maxBlack);

	void Traverse(void);
	void TraverseRec(LLTB_t *pNode, unsigned int &prev);
};
