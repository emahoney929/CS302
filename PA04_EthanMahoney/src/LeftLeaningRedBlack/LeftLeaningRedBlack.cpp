...
#include "LeftLeaningRedBlack/LeftLeaningRedBlack.h"
#include <algorithm>
#include <iostream>
...
//
void* LeftLeaningRedBlack::LookUp(unsigned int key)
{
...
/////////////////////////////////////////////////////////////////////////////
//
// findParent() - finds the parent of the key (child node) parameter.
//
void LeftLeaningRedBlack::findParent(LLTB_t* pNode, unsigned int key, int parent)
{
    if (pNode == nullptr)
        return;
 
    // If the child node is found execute statement
    if (pNode->Ref.Key == key) {
 
        // Prints the parent of the cild node that was found and it's color
		if(parent)
		{
        	std::cout << "The parent of this insertion is the node with value " << parent << std::endl;

			bool color = false;
			if(isRed(m_pRoot, parent, color))
			{
				std::cout << "The parent Node is Red\n" << std::endl;
			}
			else
			{
				std::cout << "The parent Node is Black\n" << std::endl;
			}
			
		}
		else
		{
			std::cout << "After rotation, this node is now the root. No Parent Node\n" << std::endl;
		}
        ...
        // Recursive traversal
        // of the current node
        // The previous node becomes the new parent
        findParent(pNode->pLeft, key, pNode->Ref.Key);
        findParent(pNode->pRight, key, pNode->Ref.Key);
    }
}

/////////////////////////////////////////////////////////////////////////////
//
// isRed() - Method to check to see if a given node is red or not.
//
bool LeftLeaningRedBlack::isRed(LLTB_t* pNode, unsigned int key, bool& colorChoice)
{
	if(pNode != nullptr)
	{
		if(pNode->Ref.Key == key)
			return colorChoice = pNode->IsRed;
	
		isRed(pNode->pLeft, key, colorChoice);
		isRed(pNode->pRight, key, colorChoice);
	}
...
/////////////////////////////////////////////////////////////////////////////
//
// getRoot() const - Method returns a pointer to the root node of the tree
//
LLTB_t* LeftLeaningRedBlack::getRoot() const
{
	return m_pRoot;
}
...
//	Delete()
//
void LeftLeaningRedBlack::Delete(const unsigned int key)
{
	if (nullptr != m_pRoot) {
...
//	DeleteRec()
//
LLTB_t* LeftLeaningRedBlack::DeleteRec(LLTB_t *pNode, const unsigned int key)
{
	if (key < pNode->Ref.Key) {
		if (nullptr != pNode->pLeft) {
...
{
	if (nullptr != m_pRoot) {
		unsigned int minBlack = 0xFFFFFFFF;
...
//
void LeftLeaningRedBlack::SanityCheckRec(LLTB_t *pNode, bool isParentRed, unsigned int blackDepth, unsigned int &minBlack, unsigned int &maxBlack)
{
	if (pNode->IsRed) {
	...
	if (nullptr != pNode->pLeft) {
		...
		minBlack = std::min(minBlack, blackDepth);
		maxBlack = std::max(maxBlack, blackDepth);
	}
...
//
unsigned int LeftLeaningRedBlack::KeyCount(void)
{
...
//	KeyCountRec()
//
unsigned int LeftLeaningRedBlack::KeyCountRec(LLTB_t *pNode)
{
...
void LeftLeaningRedBlack::LeafDepth(void)
{
	...
	// depth.
	unsigned int ary[64];
	for (unsigned int i = 0; i < sizeof(ary); ++i) {
		ary[i] = 0;
	}
	...
	if (nullptr != m_pRoot) {
		maxDepth = LeafDepthRec(m_pRoot, 0, ary, sizeof(ary), 0, minBlack, maxBlack);
	}
...
//	LeafDepthRec()
//
unsigned int LeftLeaningRedBlack::LeafDepthRec(LLTB_t *pNode, unsigned int depth, unsigned int ary[], unsigned int depthLimit, unsigned int blackDepth, unsigned int &minBlack, unsigned int &maxBlack)
{
...
//	TraverseRec()
//
void LeftLeaningRedBlack::TraverseRec(LLTB_t *pNode, unsigned int &prev)
{
